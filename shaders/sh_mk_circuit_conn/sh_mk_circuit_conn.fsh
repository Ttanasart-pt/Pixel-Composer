...
void main() {
	float gr = min(v_vColour.r, .999) - gradientShift;
	      gr = fract(fract(gr) + 1.);
	
	vec4 colr = v_vColour.r > 0.? colorEnd : colorStart;
	vec4 wire = wireColor * gradientEval(gr);
	
	if(blendMode == 1) colr *= wire;
	
	vec4 base = texture2D(gm_BaseTexture, v_vTexcoord);
	gl_FragColor = base * colr;
