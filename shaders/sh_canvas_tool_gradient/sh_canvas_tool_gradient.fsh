...
float pointLineProgress(vec2 p, vec2 a, vec2 b) {
	vec2 ab = b - a;
	vec2 ap = p - a;
	...
	return clamp(dot(ap, ab) / ab2, 0., 1.);
}

void main() {
	float prog = pointLineProgress(v_vTexcoord * dimension, p1, p2);
	vec4  gradColor = gradientEval(prog);
