...
uniform float     amount_curve[CURVE_MAX];
uniform int       amount_curve_use;
uniform int       amount_amount;
...
void main() {
	...
	if(amount_curve_use == 1) {
		if(axis == 0) amo *= curveEval(amount_curve, amount_amount, pos.y);
		else          amo *= curveEval(amount_curve, amount_amount, pos.x);
	}
