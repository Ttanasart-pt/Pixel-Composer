...
uniform float     brightSmooth_curve[CURVE_MAX];
uniform int       brightSmooth_curve_use;
uniform int       brightSmooth_amount;
...
uniform float     alphaSmooth_curve[CURVE_MAX];
uniform int       alphaSmooth_curve_use;
uniform int       alphaSmooth_amount;
...
void main() {
	...
	if(bright == 1) {
		...
		if(brightSmooth_curve_use == 1) 
			vBright = curveEval(brightSmooth_curve, brightSmooth_amount, vBright);
	...
	if(alpha == 1) {
		...
		if(alphaSmooth_curve_use == 1) 
			col.a = curveEval(alphaSmooth_curve, alphaSmooth_amount, col.a);
