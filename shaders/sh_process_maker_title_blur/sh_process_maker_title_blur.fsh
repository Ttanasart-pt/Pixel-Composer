...
void main() {
	...
	for( float j = -cel; j <= cel; j++ ) {
		...
		float wei  = 1. - clamp((abs(i) + abs(j)) / (size * 2.), 0., 1.);
		      wei *= clamp(abs(i + j - floor(size) * 2.), 0., 1.);
	...
	vec4  textSamp = clr;
	float prog = (v_vTexcoord.x + v_vTexcoord.y) / 2.;
	      prog = .5 + (prog - .5) * .8;
	      
	vec4  grad = gradientEval(prog);
	
	textSamp.rgb  = grad.rgb;
	// if(base.a > 0.) textSamp.a = 0.;
		
	gl_FragColor  = textSamp;
	// gl_FragColor  = clr;
