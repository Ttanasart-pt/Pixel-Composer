...
void checkGrass(in vec2 pos) { 
	if(texture2D(grassMask, pos).g < .5) return;
	
	vec4 baseC = gradientEval(pfract(random(pos + vec2(16.2681)) + gradient_shift));
	vec2 px = floor(pos * dimension);
	if(mod(px.x + px.y, 2.) < 1.) return;
