// Pillow Blend mode by TinyTexel
// https://www.shadertoy.com/view/fXsGDl
...
void main() {
	...
	vec2 fore_tex = v_vTexcoord;
	     if(tile_type == 0) fore_tex = v_vTexcoord;
	else if(tile_type == 1) fore_tex = fract(v_vTexcoord * dimension);
	...
	vec4 res = (_cFg * _cBg * (_cFg + _cBg - 2.0)) / (_cFg * _cFg - _cFg) + (_cBg * _cBg - _cBg);
