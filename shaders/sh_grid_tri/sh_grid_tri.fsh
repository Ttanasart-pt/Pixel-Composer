...
void main() {
	...
	if(mode == 0) {
		...
		uv = (_pos * vec2(1., c30) - tri.xy) + vec2(.5, 0.);
		
		if(textureTransform == 1) {
			float rx  = random(floor(tri.xy) + textureSeed / 100.);
			float ry  = random(floor(tri.xy) + textureSeed / 100. + vec2(0.4864, 0.6879));
			float ang = 0.;
			
			if(rx > textureFlip) ang += 120.;
			if(ry > textureFlip) ang += 120.;
			
			float tseed = random(floor(tri.xy) + textureSeed / 100. + vec2(0.9843, 0.1636));
		...
		uv   = fract(tri.xy / sca / vec2(dimension.x / dimension.y, c30) + .5);
		colr = sampleTexture( gm_BaseTexture, uv ) * base;
	}
	
	float _aa = 3. / max(dimension.x, dimension.y);
	gl_FragData[0] = mix(gapCol, colr, aa == 1? smoothstep(wid * 2. - _aa, wid * 2., dist) : step(wid * 2., dist));
}