...
void main() {
    ...
    if(falloff_curve_use == 1) {
        float normDis = dis / (radius + falloff);
        float curveVal = curveEval(falloff_curve, falloff_amount, normDis);
        inf = 1. - clamp(curveVal, 0., 1.);
    } else
        inf = smoothstep(radius - falloff, radius + falloff, dis);
    inf = 1. - inf;
