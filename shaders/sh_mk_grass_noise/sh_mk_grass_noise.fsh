...
float simplex(in vec2 ntx, in float Z) {
	vec2 st  = (ntx - position.xy / dimension) * mat2(cos(rotation), -sin(rotation), sin(rotation), cos(rotation)) * scale;
    vec2 p   = st;
	float _z = 1. + Z;
    vec3 xyz = vec3(p, _z);
...
void main() {
	...
	float grass = simplex(v_vTexcoord, position.z + 16.1285);
	      grass = smoothstep((1. - density) - (1. - sharpness), (1. - density) + (1. - sharpness), grass);
	
	baseCol.r *= simplex(v_vTexcoord, position.z);
	baseCol.g *= grass;
