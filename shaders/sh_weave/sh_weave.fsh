...
uniform float shading_curve[CURVE_MAX];
...
bool getWeaveAxis(in vec2 wIndex) {
	if(wtype != 2) wIndex = mod(wIndex, floor(dimension / scale));
	bool ax = false;
	
	     if(wtype == 0) ax = frandom(wIndex) > 0.5;
	else if(wtype == 1) ax = mod(wIndex.x + wIndex.y, 2.) > 0.;
	else if(wtype == 2) {
		float ang = radians(rotation);
		vec2  sm  = (wIndex + .5) * (scale / dimension) * mat2(cos(-ang), -sin(-ang), sin(-ang), cos(-ang)) + position / dimension;
		      
		vec4 wm = sampleTexture(wmap, sm);
		ax = wm.r * wm.a > .5;
	}
...
void main() {
	...
	vec4 res = color2;
	     ...
	     if(colorType == 0) res = color2;
	else if(colorType == 1) {
		res = color2;
		if(ax  && fX)        res = color3;
		if(!ax && fX && !fY) res = color3;
		
	} else if(colorType == 2) {
		bool cax = (ax && fX) || (!ax && fX && !fY);
		vec2 cid = wIndex;
		if(cax) cid.y = 0.;
		else    cid.x = 0.;
	...
	if(ax) {
		if(!fX && fY) {
			float shade = (abs(wUV.x - .5) - wx) / max(0.0001, shadeSpan - wx);
			float shadC = curveEval(shading_curve, shading_amount, shade);
			res.rgb = mix(shadeColor.rgb, res.rgb,  1. - (1. - shadC) * shading);
			
		} else {
			float off = wUV.y < .5? -1. : 1.;
			
			bool ax1 = getWeaveAxis(wIndex + vec2(0., off));
			if(!ax1) {
				float shade = (abs(wUV.y - off - .5) - wy) / max(0.0001, shadeSpan - wy);
				float shadC = curveEval(shading_curve, shading_amount, shade);
				res.rgb = mix(shadeColor.rgb, res.rgb,  1. - (1. - shadC) * shading);
			}
		}
	...
	if(!ax) {
		if(fX && !fY) {
			float shade = (abs(wUV.y - .5) - wy) / max(0.0001, shadeSpan - wy);
			float shadC = curveEval(shading_curve, shading_amount, shade);
			res.rgb = mix(shadeColor.rgb, res.rgb,  1. - (1. - shadC) * shading);
			
		} else {
			float off = wUV.x < .5? -1. : 1.;
			
			bool ax1 = getWeaveAxis(wIndex + vec2(off, 0.));
			if(ax1) {
				float shade = (abs(wUV.x - off - .5) - wx) / max(0.0001, shadeSpan - wx);
				float shadC = curveEval(shading_curve, shading_amount, shade);
				res.rgb = mix(shadeColor.rgb, res.rgb,  1. - (1. - shadC) * shading);
			}
		}
