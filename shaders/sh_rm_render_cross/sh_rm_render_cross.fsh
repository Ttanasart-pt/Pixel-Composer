...
#region -- d3d_sdf -- [1778544363.3362846]
	...
	uniform float specular[MAX_SHAPES];
	...
	#region ////=========== Texturing ============
		...
		vec3 getDiffuseColor(in int index, in vec3 coll, in vec3 norm) {
			...
			vec3 _c = diffuseColor[index].rgb;
			
			if(useTexture[index] == 0) 
				return _c;
				
			int indx = int(TEXTURE_S) + index;
			vec3 pos = irotMatrix * (coll - position[index]) * textureScale[index];
			vec3 nor = irotMatrix * norm;
			
			_c  = boxmap(indx, pos, nor, triplanar[index], textureFilter[index]).rgb;
			_c *= diffuseColor[index].rgb;
			return _c;
		}
...
void main() {
	vec3  pos;
	vec2  spa  = span * (v_vTexcoord - .5) * 2.;
	
	if(axis == 0) pos = vec3(offset, middle.x, middle.y) + vec3(0., spa.x, spa.y);
	if(axis == 1) pos = vec3(middle.x, offset, middle.y) + vec3(spa.x, 0., spa.y);
	if(axis == 2) pos = vec3(middle.x, middle.y, offset) + vec3(spa.x, spa.y, 0.);
	
	float dist = operateSceneSDF(pos);
	float rend = 1. - (dist - depth.x) / (depth.y - depth.x);
