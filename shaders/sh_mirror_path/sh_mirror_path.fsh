...
vec2 reflect2d(vec2 p, vec2 l0, vec2 l1) {
    vec2 lineDir = normalize(l1 - l0);
    vec2 normal  = vec2(-lineDir.y, lineDir.x);
    float dist   = dot(p - l0, normal);
    return p - 2. * dist * normal;
}

bool pointOnLeft(vec2 p, vec2 a, vec2 b) {
...
void main() {
	...
	for(int i = 1; i < resolution; i++) {
		...
		float dist = distanceToLine(p, p0, p1);
		if(dist < minDist) {
			minDist = dist;
			l0 = p0;
			l1 = p1;
