...
function Panel_Graph(_project = PROJECT) : PanelContent() constructor {
    ...
    #region // ---- topbar ----
		...
		topbar_node_w  = new Easer_UI();
		topbar_sett_w  = new Easer_UI();
    ...
    #region // ---- context frame ----
        ...
        toolbar_area_w         = new Easer_UI();
        context_area_w         = new Easer_UI();
    #endregion
    ...
    function dragGraph() {
        if(graph_autopan) {
            ...
            if(project.slideShow_current) 
        		project.slideShow_current.slideStep(pr);
    ...
    function dragNodes() {
    	...
    	switch(node_drag_type) {
    		case 0 : // DRAG
	            ...
	            if(mouse_lrelease() && (nx != node_drag_sx || ny != node_drag_sy)) {
	            ...
	            if(!node_drag_add && mouse_lrelease()) {
	            	node_dragging   = noone;
	            	frame_draggings = [];
	            }
    			break;
        ...
        if(mouse_lpress()) {
            ...
            node_drag_sw  = _node.w;
            node_drag_sh  = _node.h;
    ...
    function drawViewController() { //
        ...
        if(view_hovering && mouse_rpress(pFOCUS)) {
        	mouse_on_graph = false;
        	menuCall("preview_view_controller", [ 
        		menuItem("Hide view controllers", function() /*=>*/ { project.graphDisplay.show_view_control = 0; }) 
    		]);
        }
    } 
    ...
    function drawNodes() { 
        ...
        #region node list filter
	        array_foreach(project.allNodes, function(_n,i) /*=>*/ { _n.is_selecting = false; _n.region_hovering = false; return true; });
	        
	        var _node_active = array_filter(nodes_list, function(_n,i) /*=>*/ {return _n.active && _n.visible});
	        var _node_draw   = array_filter( _node_active, function(_n,i) /*=>*/ {
        ...
        #region drawController
        	if(PREFERENCES.panel_graph_show_control) {
	        	var _node_controller = array_filter(_node_active, function(_n,i) /*=>*/ {return _n.is_controller});
		        array_foreach(_node_controller, function(_n,i) /*=>*/ { 
        ...
        #region drawNodeBG
	        _frame_hovering  = frame_hovering;
	         frame_hovering  = noone;
	         frame_hoverings = [];
	        _node_frames     = array_filter(_node_draw, function(_n,i) /*=>*/ {return is(_n, Node_Frame)});
	        
	        array_foreach(_node_frames, function(_n,i) /*=>*/ { 
	        ...
	        array_foreach(_node_draw, function(_n,i) /*=>*/ { 
	        	if(is(_n, Node_Frame)) return true;
	        	...
	        	if(!array_empty(_n.inputDisplayGroup)) 
            		_n.drawJunctionGroups(_xx, _yy, __mx, __my, __gr_s, true, false, .6);
        ...
        #region ++++++++++++ interaction ++++++++++++
	        ...
	        if(mouse_on_graph && pHOVER) {
                ...
                if(mouse_lpress(_focus)) _anc = noone;
                ...
                if(NODE_DROPPER_TARGET != noone && node_hovering) {
                	...
                	if(array_valid(frame_hovering)) array_foreach(frame_hovering, function(f,i) /*=>*/ {return f.getCoveringNodes(nodes_list)});
                	...
                	if(dropper_active) {
                        ...
                        if(DOUBLE_CLICK) {
                        	...
                        	if(_ctx || !array_empty(project.globalLayer_nodes)) 
                    ...
                    if(array_valid(nodes_selecting)) array_foreach(nodes_selecting, function(n,i) /*=>*/ {return bringNodeToFront(n)});
                }
	                ...
	                if(value_focus) {
	                    ...
	                    if(value_focus.connect_type == CONNECT_TYPE.output) {
	                        ...
	                        for( var i = 0, n = array_length(value_focus.value_to); i < n; i++ ) {
	                            ...
	                            array_push(menu, menuItem(_lb, function(data) /*=>*/ { data.juncTo.removeFrom(); }, 
        ...
        #region drawActive
	        array_foreach(nodes_selecting, function(_n,i) /*=>*/ { 
	        	_n.drawActive(instanceof(_n) == FOCUS_STR); 
	        	_n.is_selecting = true; 
	        	return true; 
	        });
		...
		#region draw connections
	        ...
	        if(connection_draw_update || pHOVER) {
		        ...
		        surface_set_target(connection_surface_cc);
			        ...
			        array_foreach(_node_active, function(n,i) /*=>*/ {
			        	if(n.group != __context) return true;
			        	
			        	var _hov = n.drawConnections(connection_param, connection_draw_update);
			        	if(!is_array(_hov)) return true;
	        ...
	        surface_set_target(connection_surface);
		        ...
		        var _drawMouse = value_dragging && connection_draw_mouse != noone;
		        if(_drawMouse) {
        ...
        #region draw node
	        ...
	        if(node_surface_update) {
	        	...
	        	array_foreach(_node_draw, function(_n,i) /*=>*/ { if(_n.drawNodeBehind) _n.drawNodeBehind(__gr_x, __gr_y, __mx, __my, __gr_s); return true; });
		        ...
		        array_foreach(_node_draw, function(_n,i) /*=>*/ { if(_n.drawNodeFG) _n.drawNodeFG(__gr_x, __gr_y, __mx, __my, __gr_s, __self); return true; });
				surface_reset_target();
	        }
	        ...
	        #region drawController
	        	if(PREFERENCES.panel_graph_show_control)
			        array_foreach(_node_controller, function(_n,i) /*=>*/ {return _n.drawNodeFG(__gr_x, __gr_y, __mx, __my, __gr_s)});
	        #endregion
	    ...
	    #region highlight
	    	if(node_highlighting != undefined) {
	    		var hl = string_lower(node_highlighting);
	    		...
	    		for( var i = 0, n = array_length(_node_draw); i < n; i++ ) {
	    			...
	    			if(string_lower(_n.name) != hl && string_lower(_n.display_name) != hl) 
	    				continue;
	    			
	    			var nx = __gr_x + _n.x * __gr_s;
	        ...
	        if(nodes_select_drag) {
	            ...
	            if(nodes_select_drag == 2) {
	                ...
	                for( var i = 0, n = array_length(nodes_list); i < n; i++ ) {
	                    ...
	                    if(_sel) array_push(box_selecting, _node);
	                }
	                ...
	                for( var i = 0, n = array_length(nodes_list); i < n; i++ ) { //select inline parent
	                	...
	                	if(array_contains_ext(box_selecting, _node.nodes, true))
	                		array_push_unique(box_selecting, _node);
	                }
	                
	                nodes_selecting = [];
	                nodes_selecting = array_append(nodes_selecting, drag_box_selecting);
	                nodes_selecting = array_append(nodes_selecting, box_selecting);
        ...
        if(mouse_on_graph && !node_bg_hovering && mouse_lpress(_focus) && !graph_dragging_key && !graph_zooming_key) {
    ...
    function draggingValue() {
        ...
        if(mouse_lrelease()) value_draggings = [];
    }
    ...
    function drawJunctionConnect() {
        ...
        if(value_dragging == noone && value_focus && mouse_lpress(_focus) && !key_mod_press(ALT)) {
            ...
            if(key_mod_press(SHIFT) && value_focus.draw_group != undefined) {
            	value_draggings = [];
            	var _inpt = value_dragging.connect_type == CONNECT_TYPE.input;
            	var _list = _inpt? value_dragging.node.inputDisplayList : value_dragging.node.outputDisplayList;
            	
            	for( var i = 0, n = array_length(_list); i < n; i++ ) {
            		if(_list[i].draw_group == value_focus.draw_group)
            			array_push(value_draggings, _list[i]);
            	}
            }
    ...
    function drawToolBar() { //
        ...
        if(mrg) {
	        ...
	        gpu_set_scissor(tx + tw - toolbar_area_w.value, ty, toolbar_area_w.value, th);
        }
    ...
    function drawTopbar() { 
    	...
    	gpu_set_scissor(tx, ty, topbar_node_w.value, th);
		...
		var topW = clamp(_ww - ui(8), 0, w - pd * 3 - rightW);
		topbar_node_w.goto(topW);
    ...
    function drawSlideShow() {
        ...
        if(PREFERENCES.tutorial_mode) {
        	...
        	draw_sprite_stretched_ext(THEME.ui_panel, 2, _sl_x-pd, _sl_y-pd, slider_width+pd*2, _sl_h+pd*2, COLORS._main_accent, aa);
        }
    ...
    function drawTooltipHints() {
    	...
    	var _tx   = ui(16);
    	var _ty   = h - toolbar_height - ui(10) - pd * 2;
        ...
        if(pHOVER) { // File Drop
            ...
            if(DRAGGING) { // file dropping
                ...
                if(_node_hover && _node_hover.droppable(DRAGGING)) {
                    ...
                    _tip = "Drop on node";
                    if(mouse_lrelease()) _node_hover.onDrop(DRAGGING);
                    
                } else if(mouse_lrelease()) checkDropItem();
            }
    ...
    function disconnectAll() {
    	...
    	if(__junction_hovering.connect_type == CONNECT_TYPE.input) {
    		if(__junction_hovering.value_from)
    			__junction_hovering.removeFrom();
    			
    	} else {
    		__junction_hovering.removeTo();
...
function load_file_path(path, _x = undefined, _y = undefined) {
	...
	if(array_any(path, function(p,i) /*=>*/ {return file_is_image(p)})) 
		type = "image";
