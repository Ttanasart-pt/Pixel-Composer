...
function Node_Displace(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	newInput(9, nodeValue_Slider(  "Mix",        self, 1));
	...
	newInput( 3, nodeValue_Float(   "Strength",       self, 1)).setMappable(15);
	newInput(15, nodeValue_Surface( "Strength map",   self)).setVisible(false, false);
	newInput( 4, nodeValue_Slider(  "Mid value",      self, 0)).setTooltip("Brightness value to be use as a basis for 'no displacement'.");
	...
	newInput( 2, nodeValue_Vec2( "Position",      self, [ 1, 0 ] )).setTooltip("Vector to displace the pixel by.").setUnitRef(function(i) /*=>*/ {return getDimension(i)});
	
	////- Iterate
	
	newInput( 6, nodeValue_Bool(        "Iterate",       self, false));
