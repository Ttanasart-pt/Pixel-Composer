...
function Node_Displace(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	newInput( 3, nodeValue_Float(   "Strength",   1  )).setMappable(15).setCurvable(25).setHotkey("S").setPieMenu();
	newInput( 4, nodeValue_Slider(  "Mid Value", .5  )).setMappable(24).setTooltip("Brightness value to be use as a basis for 'no displacement'.").setPieMenu();
	...
	static drawOverlay = function(hover, active, _x, _y, _s, _mx, _my, _params) { 
		...
		drawOverlayInput(inputs[3].drawOverlay(w_hoverable, active, _cx, _cy, _s, _mx, _my, 0, _dim[0] / 2));
		
		switch(_mode) {
			case 0 : drawOverlayInput(inputs[ 2].drawOverlay(w_hoverable, active, _cx, _cy, _s, _mx, _my, 1)); break;
			
			case 5 : 
			case 6 : drawOverlayInput(inputs[26].drawOverlay(w_hoverable, active, _x,  _y,  _s, _mx, _my, 1)); break;
		}
	...
	static processData = function(_outSurf, _data, _array_index) {
		...
		_outSurf = mask_apply_input(_surf, _outSurf, _data[8], _data[9], inputs[8]);
