function Node_Refract(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	input_display_list = [ 1, 2, 
		...
		[ "Map",      false ],  7,  8, 
		[ "Refract",  false ],  9, 12, 10, 13, 11, 14, 15, 
	];
	...
	static processData = function(_outSurf, _data, _array_index = 0) { 
		...
		surface_set_shader(_outSurf, sh_refract);
			...
			shader_set_s( "refNormalSurf", _normS );
			shader_set_s( "refDepthSurf",  _deptS );
			
			shader_set_2( "dimension",     _dim  );
			shader_set_f( "perspective",   _pres );
			shader_set_m( "depth",         _dept, _data[12], inputs[ 9] );
			shader_set_m( "offset",        _dist, _data[13], inputs[10] );
			shader_set_m( "IOR",           _ior,  _data[14], inputs[11] );
