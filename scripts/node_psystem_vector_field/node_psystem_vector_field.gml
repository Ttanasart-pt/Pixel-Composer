function Node_pSystem_Vector_Field(_x, _y, _group = noone) : Node(_x, _y, _group) constructor {
	name  = "Vector Field";
	icon  = THEME.vfx;
	...
	static update = function(_frame = CURRENT_FRAME) {
		...
		var vectr_samp = new Surface_sampler(_vect);
		...
		repeat(_partAmo) {
			...
			var _sampClr = vectr_samp.getPixel(sx, sy);
			var _sampR   = _color_get_red(_sampClr);
			var _sampG   = _color_get_green(_sampClr);
			
			var _dx = (_sampR - _midp) * _ints_cur;
			var _dy = (_sampG - _midp) * _ints_cur;
			
			buffer_poke( _partBuff, _start + PSYSTEM_OFF.posx, buffer_f64, _px + _dx );
			buffer_poke( _partBuff, _start + PSYSTEM_OFF.posy, buffer_f64, _py + _dy );
		}
