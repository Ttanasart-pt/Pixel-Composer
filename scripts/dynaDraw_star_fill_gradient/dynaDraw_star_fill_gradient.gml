function dynaDraw_star_fill_gradient() : dynaDraw() constructor {
	...
	static draw = function(_x = 0, _y = 0, _sx = 1, _sy = 1, _ang = 0, _col = c_white, _alp = 1) {
		...
		draw_primitive_begin(pr_trianglelist);
			for( var i = 0; i < _sd; i++ ) {
				...
				draw_vertex_color(_x + lengthdir_x(sw,  a0), _y + lengthdir_y(sh,  a0), c_black, _alp);
				draw_vertex_color(_x + lengthdir_x(swi, a1), _y + lengthdir_y(shi, a1), c_black, _alp);
				
				draw_vertex_color(_x, _y, _col, _alp);
				draw_vertex_color(_x + lengthdir_x(swi, a1), _y + lengthdir_y(shi, a1), c_black, _alp);
				draw_vertex_color(_x + lengthdir_x(sw,  a2), _y + lengthdir_y(sh,  a2), c_black, _alp);
			}
		draw_primitive_end();
