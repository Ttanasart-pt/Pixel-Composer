function Panel_Custom_Slider(_data) : Panel_Custom_Element(_data) constructor {
	...
	array_append(editors, [
		...
		Simple_Editor("Clamp Range", new checkBox(function() /*=>*/ { clamp_to_range = !clamp_to_range; } ),            function() /*=>*/ {return clamp_to_range}, function(t) /*=>*/ { clamp_to_range = t; } ), 
	...
	static draw = function(panel, _m) {
		...
		if(dragging) {
		    ...
		    if(clamp_to_range) val = clamp(val, range[0], range[1]);
