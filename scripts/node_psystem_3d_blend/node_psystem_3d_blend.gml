function Node_pSystem_3D_Blend(_x, _y, _group = noone) : Node_3D(_x, _y, _group) constructor {
	name  = "Blend";
	icon  = THEME.vfx;
	color = COLORS.node_blend_vfx;
	setDrawIcon(s_node_psystem_3d_blend);
	...
	static update = function(_frame = CURRENT_FRAME) { 
		...
		var _alph_use  = _data[10], _alph_curved = curve_alph != undefined;
		
		var _sold_use  = _data[12];
		var _sold_mode = _data[14];
		var _sold      = _data[13];
		
		var _grad_use  = _data[ 8];
		var _grad_mode = _data[ 3];
		var _grad      = _data[ 4];
		
		var _palt_use  = _data[ 9];
		var _palt_mode = _data[ 7];
		var _palt      = _data[ 5], _palt_curved = inputs[5].attributes.curved && curve_palt != undefined;
