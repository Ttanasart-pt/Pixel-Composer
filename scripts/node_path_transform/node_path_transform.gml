function Node_Path_Transform(_x, _y, _group = noone) : Node(_x, _y, _group) constructor {
	...
	static getSegmentCount	= function(ind = 0) { return struct_has(path, "getSegmentCount")? path.getSegmentCount(ind)	: 0;  }
	static getLength		= function(ind = 0) { return struct_has(path, "getLength")?		  path.getLength(ind)		: 0;  }
	static getAccuLength	= function(ind = 0) { return struct_has(path, "getAccuLength")?	  path.getAccuLength(ind)	: []; }
