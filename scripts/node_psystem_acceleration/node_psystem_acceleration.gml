function Node_pSystem_Acceleration(_x, _y, _group = noone) : Node(_x, _y, _group) constructor {
	...
	static update = function(_frame = CURRENT_FRAME) {
		...
		repeat(_partAmo) {
			...
			var _vpx    = buffer_peek( _partBuff, _start + PSYSTEM_OFF.velpx,  buffer_f64  );
			var _vpy    = buffer_peek( _partBuff, _start + PSYSTEM_OFF.velpy,  buffer_f64  );
			...
			buffer_poke(_partBuff, _start + PSYSTEM_OFF.posx, buffer_f64, _ppx + _dx );
			buffer_poke(_partBuff, _start + PSYSTEM_OFF.posy, buffer_f64, _ppy + _dy );
