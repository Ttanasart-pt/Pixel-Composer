...
function draw_text_match_range(_x, _y, _text, _range, _scale = 1) {
	...
	repeat(aa) {
		var ch = string_char_at(_text, j);
		draw_set_color(j < _ranLen && _range[j]? COLORS._main_accent : cc);
...
function string_full_match(str, key)  { return string_pos(key, str)? 1 :  -9999; }
function string_match_lower(str, key) { return string_pos(string_lower(str), string_lower(key)) != 0; }
