function Node_MK_Tree_Branch(_x, _y, _group = noone) : Node(_x, _y, _group) constructor {
	...
	newInput(52, nodeValue_EButton(  "Array Selection", 0           )).setChoices([ "Ordered", "Random" ]);
		...
		for( var i = 0, n = array_length(_tree); i < n; i++ ) {
			...
			repeat(_amo) {
				...
				switch(_refl) {
					...
					case 2 : _reflected = (_spawnIndx % 2) == _refOrd; break;
				}
