function Node_Maze_Pattern(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	newInput( 4, nodeValue_Int(     "Max Iteration", -1 )).setPieMenu();
	...
	newInput( 7, nodeValue_Gradient( "Path Color",     gra_white )).addShift(11);
	...
	input_display_list = [ 3, 
		...
		[ "Rendering", false ],  6, [7, true],  11, -1,  8, 
	];
	...
	static processData = function(_outSurf, _data, _array_index = 0) { 
		...
		var ww = max(1, ceil(_dim[0]));
		var hh = max(1, ceil(_dim[1]));
		
		var ox = clamp(round(_orig[0]), 1, ww - 1);
		var oy = clamp(round(_orig[1]), 1, hh - 1);
