function Node_Dilate(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	newInput(2, nodeValue_Slider( "Strength",  1, [-3,3,.01] )).setHotkey("S").setMappable(11).setCurvable(15, CURVE_DEF_01).setPieMenu();
	newInput(3, nodeValue_Float(  "Radius",   .5             )).setHotkey("R").setMappable(12).setUnitSimple().setPieMenu();
	// input 16
	...
	static drawOverlay = function(hover, active, _x, _y, _s, _mx, _my, _params) { 
		...
		drawOverlayInput(inputs[2].drawOverlay(w_hoverable, active, _cx, _cy, _s, _mx, _my, 90, _dim[0] / 2, 2));
