function canvas_s_selector_shape_freeform() : canvas_s_tool() constructor {
	icon       = THEME.canvas_tools_freeform_selection;
	tooltip    = "Freeform Selector";
					...
					case 0 : 
						if(array_length(freeform_shape) > 3) {
							...
							surface_set_target(temp_surface);
								...
								draw_primitive_end();
								draw_surface_safe(selection_mask);
							surface_reset_target();
						}
						break;
	...
	function drawOutline(_x, _y, _s) {
		if(mouse_holding) draw_surface_ext_safe(selection_mask, _x, _y, _s, _s, 0, c_white, 1);
	}
}