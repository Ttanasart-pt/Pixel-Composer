function Node_pSystem_3D_Acceleration(_x, _y, _group = noone) : Node_3D(_x, _y, _group) constructor {
	name  = "Accelerate";
	icon  = THEME.vfx;
	color = COLORS.node_blend_vfx;
	node_draw_icon = s_node_psystem_3d_acceleration;
	...
	static reset = function() {
		curve_acel = new curveMap(getInputData(4));
	}
	...
	static update = function(_frame = CURRENT_FRAME) { 
		...
		var _seed = _data[ 2];
		var _acel = _data[ 3], _acel_curved = inputs[3].attributes.curved && curve_acel != undefined;
		...
		repeat(_partAmo) {
			...
			_vz = max(_vz * (1 + _acel_cur * _mask), 0);
