function Node_MK_Facet(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	////- =Shape
	newInput(11, nodeValue_EScroll( "Shpe", 0, [ "Rectangle", "Ellipse" ] ));
	newInput( 1, nodeValue_Area(    "Area", AREA_DEF_REF, false )).setUnitSimple();
	newInput( 5, nodeValue_Slider(  "Trim", 0 ));
	
	////- =Render
	newInput( 4, nodeValue_Color(    "Base Color",    ca_white ));
	...
	facet_builder = new Inspector_Custom_Renderer(function(_x, _y, _w, _m, _hover, _focus) {
		...
		for( var i = 0; i < _amo; i++ ) {
			...
			// draw_sprite_stretched_ext(THEME.ui_panel, 0, _fx, _fy, _w, _hg, CDEF.main_dkblack, 1);
			// if(_hv) draw_sprite_stretched_add(THEME.ui_panel, 1, _fx, _fy, _w, _hg, c_white, .25);
			...
			_edt.drawParam(new widgetParam(_ffx, _ffy, _ffw, _ffh, _fc, undefined, _m, facet_builder.rx, facet_builder.ry));
			
			var _fww = _ffw / 4;
			var _hov = _hover && point_in_rectangle(_m[0], _m[1], _ffx, _ffy, _fww, _ffh);
	...
	static processData = function(_outData, _data, _array_index = 0) { 
		...
		surface_set_shader(_outData, sh_mk_facet_render);
			...
			draw_surface_ext(temp_surface[!__bg_index], 0, 0, 1, 1, 0, _colr, 1);
		surface_reset_shader();
