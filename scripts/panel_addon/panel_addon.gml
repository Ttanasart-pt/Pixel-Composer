		...
		sc_addon = new scrollPane(w - padding * 2, h - (padding * 2 + ui(40)), function(_y, _m) {
			...
			for( var i = 0, n = array_length(arr); i < n; i++ ) {
				...
				if(_hover && point_in_rectangle(_m[0], _m[1], chx0, chy0, chx1, chy1)) {
					...
					if(!_act)			setTOOLTIP(__txt("panel_addon_not_activated", "Not activated"));
					else if(!_onStart)	setTOOLTIP(__txt("panel_addon_activated",     "Activated"));
					else				setTOOLTIP(__txt("panel_addon_run_on_start",  "Run on start"));
