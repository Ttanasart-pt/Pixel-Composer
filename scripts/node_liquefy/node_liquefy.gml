...
function Node_Liquefy(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	static drawOverlay = function(hover, active, _x, _y, _s, _mx, _my, _params) { 
		...
		switch(_type) {
			case LIQUEFY_TYPE.push :
				...
				if(_path == noone) {
					...
					drawOverlayInput(inputs[_ind +  2].drawOverlay(w_hoverable, active, _x, _y, _s, _mx, _my));
					drawOverlayInput(inputs[_ind +  1].drawOverlay(w_hoverable, active, _x, _y, _s, _mx, _my));
					drawOverlayInput(inputs[_ind + 10].drawOverlay(w_hoverable, active, qx, qy, _s, _mx, _my, 0, 1, 1));
					...
					drawOverlayInput(inputs[_ind + 10].drawOverlay(w_hoverable, active, ox, oy, _s, _mx, _my));
				}
				break;
			...
			case LIQUEFY_TYPE.bloat :
				...
				if(fal != 0) {
					...
					drawOverlayInput(inputs[_ind + 5].drawOverlay(w_hoverable, active, fx, py, _s, _mx, _my, 0, 1, 2));
				} else 
					drawOverlayInput(inputs[_ind + 5].drawOverlay(w_hoverable, active, fx + ui(6), py, _s, _mx, _my, 0, 1, 2));
		...
		draw_set_color(COLORS._main_accent);
		draw_circle(px, py, rad, true);
		drawOverlayInput(inputs[_ind + 3].drawOverlay(w_hoverable, active, px, py, _s, _mx, _my, 0, 1, 1));
		return w_hovering;
	}
