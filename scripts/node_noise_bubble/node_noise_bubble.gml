...
function Node_Noise_Bubble(_x, _y, _group = noone) : Node_Shader_Generator(_x, _y, _group) constructor {
	...
	newInput(1, nodeValue_Float("Density", 0.5 ))
	...
	newInput(4, nodeValue_Float("Thickness", 0 ))
		.setDisplay(VALUE_DISPLAY.slider);
		addShaderProp(SHADER_UNIFORM.float, "thickness");
		
	newInput(5, nodeValue_Enum_Button("Mode",  0 , [ "Line", "Fill" ] ));
		addShaderProp(SHADER_UNIFORM.integer, "mode");
		
	newInput(6, nodeValue_Slider_Range("Opacity", [ 0., 1. ] ));
		addShaderProp(SHADER_UNIFORM.float, "alpha");
		
	newInput(7, nodeValue_Enum_Scroll("Blend Mode",  0 , [ "Max", "Add" ] ));
