function NodeObject(_name, _spr, _create, tags = []) constructor {
	...
	function build(_x, _y, _param = "") {
		var group = PANEL_GRAPH.getCurrentContext();
		var _node = createNode[0]? new createNode[1](_x, _y, group, _param) : createNode[1](_x, _y, group, _param);
		return _node;
	}
}
...
#region nodes
	...
	addNodeObject(group, "Input",	s_node_input,	"Node_Group_Input",		[1, Node_Group_Input]);
	addNodeObject(group, "Output",	s_node_output,	"Node_Group_Output",	[1, Node_Group_Output]);
	...
	addNodeObject(iter, "Input",	s_node_input,			"Node_Iterator_Input",	[1, Node_Iterator_Input]);
	addNodeObject(iter, "Output",	s_node_output,			"Node_Iterator_Output",	[1, Node_Iterator_Output]);
	
	var vfx = ds_list_create();
	addNodeCatagory("VFX", vfx);
	addNodeObject(vfx, "Input",			s_node_input,		"Node_Group_Input",		[1, Node_Group_Input]);
	...
	addNodeObject(number, "Math",			s_node_math,			"Node_Math",			[0, Node_create_Math], ["add", "subtract", "multiply", "divide", "power", "modulo", "round", "ceiling", "floor", "sin", "cos", "tan"]);
	addNodeObject(number, "Statistic",		s_node_statistic,		"Node_Statistic",		[0, Node_create_Statistic], ["sum", "average", "mean", "median", "min", "max"]);
	addNodeObject(number, "Array",			s_node_array,			"Node_Array",			[1, Node_Array]);
	addNodeObject(number, "Array length",	s_node_array_length,	"Node_Array_Length",	[1, Node_Array_Length]);
	addNodeObject(number, "Array get",		s_node_array_get,		"Node_Array_Get",		[1, Node_Array_Get]);
	addNodeObject(number, "Number",			s_node_number,			"Node_Number",			[1, Node_Number]);
	addNodeObject(number, "Vector2",		s_node_vec2,			"Node_Vector2",			[1, Node_Vector2]);
	addNodeObject(number, "Vector3",		s_node_vec3,			"Node_Vector3",			[1, Node_Vector3]);
	addNodeObject(number, "Vector4",		s_node_vec4,			"Node_Vector4",			[1, Node_Vector4]);
	addNodeObject(number, "Vector split",	s_node_vec_split,		"Node_Vector_Split",	[1, Node_Vector_Split]);
	addNodeObject(number, "Unicode",		s_node_unicode,			"Node_Unicode",			[1, Node_Unicode]);
	addNodeObject(number, "Path",			s_node_path,			"Node_Path",			[1, Node_Path]);
	addNodeObject(number, "Area",			s_node_area,			"Node_Area",			[1, Node_Area]);
...
#region node function
	function nodeLoad(_data, scale = false) {
		...
		var funs = NODE_CREATE_FUCTION[? _type];
		var _node = funs[0]? new funs[1](_x, _y) : funs[1](_x, _y);
