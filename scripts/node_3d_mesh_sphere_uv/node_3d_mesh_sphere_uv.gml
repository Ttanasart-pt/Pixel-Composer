function Node_3D_Mesh_Sphere_UV(_x, _y, _group = noone) : Node_3D_Mesh(_x, _y, _group) constructor {
	...
	newInput(in_mesh + 0, nodeValue_Int("Horizontal Slices", 8 ))
		.setValidator(VV_min(2));
	
	newInput(in_mesh + 1, nodeValue_Int("Vertical Slices", 16 ))
		.setValidator(VV_min(3));
	
	newInput(in_mesh + 2, nodeValue_D3Material("Material", new __d3dMaterial()))
		.setVisible(true, true);
	
	newInput(in_mesh + 3, nodeValue_Bool("Smooth Normal", false ));
	
	newInput(in_mesh + 4, nodeValue_Enum_Scroll("Projection", 0, [ "Lambert", "Equirectangular" ] ));
