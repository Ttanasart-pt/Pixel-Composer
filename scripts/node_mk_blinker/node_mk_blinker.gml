function Node_MK_Blinker(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	newInput( 3, nodeValue_Slider(  "Amount",         .5        ));
	newInput( 4, nodeValue_Palette( "Target Colors", [ca_black] ));
	newInput( 5, nodeValue_Palette( "Light Colors",  [ca_white] ));
	newInput( 7, nodeValue_Slider(  "Tolerance",      .1        ));
	
	////- =Glow
	newInput( 8, nodeValue_Bool(    "Glow",      false       ));
	newInput( 9, nodeValue_Slider(  "Size",      4, [1,8,.1] ));
	newInput(10, nodeValue_Slider(  "Strength", .5           ));
	// inputs 11
