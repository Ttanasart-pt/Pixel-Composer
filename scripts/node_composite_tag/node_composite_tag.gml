function Node_Composite_Tag(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	newOutput(0, nodeValue_Output("Surface Out", VALUE_TYPE.dynaSurface, noone));
	...
	function createNewInput(index = array_length(inputs)) {
		...
		newInput(index, nodeValue_Surface("Surface")).setVisible(true, true);
	...
	static processData = function(_outSurf, _data, _array_index = 0) { 
        ...
        for( var i = 0; i < _amo; i++ ) {
			...
			surface_set_shader(temp_surface[bg], noone, true, BLEND.over);
				draw_surface_blend_ext(temp_surface[!bg], _surface, _xx, _yy);
