#region
	FN_NODE_CONTEXT_INVOKE {
		addHotkey("Node_Herringbone_Tile", "Render Type > Toggle", "R", MOD_KEY.none, function() /*=>*/ { GRAPH_FOCUS _n.inputs[7].setValue((_n.inputs[7].getValue() + 1) % 3); });
	});
#endregion
...
function Node_Herringbone_Tile(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	static drawOverlay = function(hover, active, _x, _y, _s, _mx, _my, _params) { 
		...
		drawOverlayInput(inputs[ 2].drawOverlay(w_hoverable, active, px, py, _s, _mx, _my, 1, [1,1], _rot    ));
		drawOverlayInput(inputs[19].drawOverlay(w_hoverable, active, _x, _y, _s, _mx, _my, getInputSingle(0) ));
	...
	static processData = function(_outSurf, _data, _array_index) {
		...
		surface_set_shader(_outSurf, sh_herringbone_tile);
			...
			shader_set_2( "textureAngle",     _data[20] );
			shader_set_4( "textureScale",     _data[26] );
			shader_set_f( "textureFlip",      _data[16] );
			
			shader_set_f( "gradient_shift",   _data[18] );
