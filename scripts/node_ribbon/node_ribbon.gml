function Node_Ribbon(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	static drawOverlay = function(hover, active, _x, _y, _s, _mx, _my, _params) { 
		drawOverlayInput(inputs[2].drawOverlay(hover, active, _x, _y, _s, _mx, _my, _params));
	...
	static processData = function(_outSurf, _data, _array_index = 0) { 
		...
		shader_set(sh_draw_ribbon);
			...
			for( var i = 0; i <= _samp; i++ ) {
				prg = _invp? 1 - i * t : i * t;
				prg = lerp(_rang[0], _rang[1], prg) + _shft;
				
				_path.getPointRatio(_loop? pfract(prg) : clamp(prg, 0, .999), 0, __p );
