#region functions
	...
	function d3d_path_tool_position(_node) : ToolObject() constructor {
		...
		function drawOverlay3D(active, _mx, _my, _params) {
...
function Node_Path_3D(_x, _y, _group = noone) : Node(_x, _y, _group) constructor {
	...
	static newAnchor = function(_x=0, _y=0, _z=0, _dxx=0, _dxy=0, _dxz=0, _dyx=0, _dyy=0, _dyz=0) { 
		return [ _x, _y, _z, _dxx, _dxy, _dxz, _dyx, _dyy, _dyz, 0 ]; 
	}
	...
	static drawOverlay3D = function(active, _mx, _my, _params) {
		...
		if(isUsingTool()) {
			...
			if(_tool != noone) {
				_tool.drawOverlay3D(active, _mx, _my, _params);
