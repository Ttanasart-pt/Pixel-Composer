...
function Panel_Animation() : PanelContent() constructor {
	...
	function drawAnimationControl(panel) {
		...
		if(dope_sheet_h > 8) {
				...
				#region bg
					...
					for( var i = 0; i < ds_list_size(anim_properties); i++ ) {
						...
						if(_node != prop.node) {
							...
							draw_sprite_stretched_ext(s_ui_panel_bg, 0, 0, key_y - ui(10), bar_total_w, ui(20), c_ui_blue_grey, 1);
							key_y += ui(22);
						...
						if(prop.node.anim_show) {
							...
							if(prop.animator.show_graph) {
								draw_sprite_stretched_ext(s_ui_panel_bg, 0, 0, key_y - ui(4), bar_total_w, graph_h, c_ui_blue_ltgrey, 1);
								key_y += graph_h + ui(8);
								...
								#region easing line
									if(key.ease_in > 0) {
										...
										if(pHOVER && point_in_circle(msx, msy, _tx, key_y, ui(6))) {
											...
											draw_sprite_ui_uniform(s_timeline_keyframe, 2, _tx, key_y, 1, c_ui_blue_ltgrey);
									} if(key.ease_out > 0) {
										draw_set_color(c_ui_blue_dkgrey);
										...
										if(pHOVER && point_in_circle(msx, msy, _tx, key_y, ui(6))) {
											...
											draw_sprite_ui_uniform(s_timeline_keyframe, 3, _tx, key_y, 1, c_ui_blue_ltgrey);
									}
								#endregion
					...
					#region keyframes
						...
						for( var i = 0; i < ds_list_size(anim_properties); i++ ) {
							...
							for(var k = 0; k < ds_list_size(prop.animator.values); k++) {
								...
								if(!prop.node.anim_show) {
									draw_sprite_ui_uniform(s_timeline_keyframe, 0, t, key_y_node, 1, c_ui_blue_grey);
									continue;
								}
								...
								if(ds_list_exist(keyframe_selecting, keyframe)) 
									draw_sprite_ui_uniform(s_timeline_keyframe_selecting, 0, t, key_y, 1, c_ui_orange);
					...
					for( var i = 0; i < ds_list_size(anim_properties); i++ ) {
						...
						if(_node != prop.node) {
							...
							if(pHOVER && point_in_circle(msx, msy, tx, key_y - 1, ui(10))) {
								...
								draw_sprite_ui_uniform(s_animate_node_go, 0, tx, key_y - 1, 1, c_ui_blue_grey);
							
							draw_sprite_ui_uniform(s_arrow_16, prop.node.anim_show? 3 : 0, ui(10), key_y, 1, c_ui_blue_grey);
						...
						if(pHOVER && point_in_circle(msx, msy, tx, ty, ui(6))) {
							...
							draw_sprite_ui_uniform(s_prop_keyframe, 1, tx, ty, 1, c_ui_orange);
						...
						if(isGraphable(prop.type)) {
							...
							if(pHOVER && point_in_circle(msx, msy, tx, ty, ui(8))) {
								...
								draw_sprite_ui_uniform(s_timeline_graph, 1, tx, ty, 1, prop.animator.show_graph? c_ui_orange : c_ui_blue_grey);
						}
						...
						if(pHOVER && point_in_circle(msx, msy, ui(22), key_y - 1, ui(10))) {
							...
							draw_sprite_ui_uniform(s_timeline_clock, 1, ui(22), key_y - 1, 1, c_ui_blue_grey);
			...
			draw_sprite_stretched_ext(s_ui_panel_bg, 0, ui(8), ui(8), tool_width, dope_sheet_h, c_ui_blue_white, 1);
		...
		surface_set_target(timeline_surface);	
				...
				if(inspecting && inspecting.use_cache) {
					for(var i = 0; i < ANIMATOR.frames_total; i++) {
						if(i >= array_length(inspecting.cached_output)) {
							//TODO: Deal with cache index overflow
						} else {
			...
			#region summary
				...
				for( var i = 0; i < ds_list_size(anim_properties); i++ ) {
					...
					for(var k = 0; k < ds_list_size(prop.animator.values); k++) {
						var t = prop.animator.values[| k].time * ui(timeline_scale) + timeline_shift;
						draw_sprite_ui_uniform(s_timeline_keyframe, 1, t, key_y, 1, c_ui_blue_grey);
					}
				}
			#endregion
