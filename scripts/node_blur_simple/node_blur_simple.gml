...
function Node_Blur_Simple(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	////- =Blur
	newInput( 1, nodeValue_Float(   "Size",   8 )).setHotkey("S").setValidator(VV_min(0)).setUnitSimple(false).setPieMenu();
	...
	static processData = function(_outSurf, _data, _array_index) {	
		...
		_outSurf = mask_apply_input(_data[0], _outSurf, _msk, _mix, inputs[6]);
