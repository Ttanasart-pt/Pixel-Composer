...
function Node_Armature(_x, _y, _group = noone) : Node(_x, _y, _group) constructor {
	...
	static drawOverlay_Transform = function(hover, active, _x, _y, _s, _mx, _my, _params) { 
		...
		if(bone_transform_type >= 0) {
			...
			if(_hov_type >= -1 && mouse_lpress(active)) {
		...
		switch(_tool) {
			...
			case "Mirror Bones" : 
				...
				if(mirroring_bone != noone) {
					...
					if(_bne.parent && mouse_lpress(active)) {
						recordAction(ACTION_TYPE.struct_modify, bones)
							.setName($"Mirror bone")
							.setRef(self);
			...
			default : // move bones
				if(anchor_selecting == noone) break;
				...
				} else if(_typ == ARMATURE_DRAG_TYPE.rotate) { // Rotate
					var orig = _bne.getHead();
					var _rx = _x + _s * orig.x;
					var _ry = _y + _s * orig.y;
