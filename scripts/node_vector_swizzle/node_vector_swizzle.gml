function Node_Vector_Swizzle(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	newInput( 0, nodeValue(      "Vector", self, CONNECT_TYPE.input, VALUE_TYPE.float, [])).setArrayDepth(1).setVisible(true, true);
	newInput( 1, nodeValue_Text( "Swizzle", "yxz" ));
	
	newOutput(0, nodeValue_Output("Result", VALUE_TYPE.float, [] ));
