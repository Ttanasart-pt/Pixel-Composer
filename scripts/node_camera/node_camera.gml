function Node_Camera(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	newInput(6, nodeValue_Dimension(self));
	newInput(0, nodeValue_Vec2(   "Focus Center", self, [ .5, .5 ])).setUnitRef(function() /*=>*/ {return getInputData(6)}, VALUE_UNIT.reference);
	newInput(1, nodeValue_Slider( "Zoom",         self, 1, [ 0.01, 4, 0.01 ]));
	...
	static createNewInput = function(index = array_length(inputs)) {
		...
		newInput(index + 2, nodeValue_Vec2(        $"Position {_s}",    self, [0,0])).setUnitRef(function(i) /*=>*/ {return getDimension(i)});
		newInput(index + 3, nodeValue_Enum_Scroll( $"Oversample {_s}",  self, 0, __enum_array_gen(["Empty ", "Repeat ", "Repeat X", "Repeat Y"], s_node_camera_repeat)));
		newInput(index + 4, nodeValue_Vec2(        $"Parallax {_s}",    self, [0,0]));
		newInput(index + 5, nodeValue_Float(       $"Depth {_s}",       self, 0));
	...
	static drawOverlay = function(hover, active, _x, _y, _s, _mx, _my, _snx, _sny, _params) {
		...
		var hv = inputs[6].drawOverlay(w_hoverable, active, _px, _py, _s, _mx, _my, _snx, _sny, 0, [.5, .5]); OVERLAY_HV
