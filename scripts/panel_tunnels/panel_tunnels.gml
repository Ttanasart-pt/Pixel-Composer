function Panel_Tunnels() : PanelContent() constructor {
	...
	sc_tunnel = new scrollPane(w - padding * 2, h - padding * 2 - ui(28 + 40), function(_y, _m) {
		...
		for( var i = 0, n = array_length(arr); i < n; i++ ) {
			...
			if(buttonInstant_Pad(THEME.button_hide_fill, bx, by, bs, bs, _m, _hov, _foc, _txt, THEME.cross, 0, _delc) == 2)
				_delNode = node;
			bx -= bs + ui(2);
			
			_txt = __txtx("panel_node_goto", "Go to Node");
			if(buttonInstant_Pad(THEME.button_hide_fill, bx, by, bs, bs, _m, _hov, _foc, _txt, THEME.node_goto) == 2)
				graphFocusNode(node);
			bx -= bs + ui(2);
			...
			draw_sprite_ui(THEME.tunnel, 0, ui(4 + 16), _y + hg / 2, .75, .75, 0, col, .5 + node.open * .5);
	...
	function drawContent(panel) {
		...
		sc_tunnel.verify(w - padding * 2, h - padding * 2 - th - ui(28 + 8));
		sc_tunnel.setFocusHover(pFOCUS, pHOVER);
		sc_tunnel.drawOffset(px, py + th + ui(8), mx, my);
