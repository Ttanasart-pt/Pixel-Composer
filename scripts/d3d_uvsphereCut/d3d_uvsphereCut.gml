function __3dUVSphereCut(_radius = 0.5, _hori = 16, _vert = 8, _smt = false) : __3dObject() constructor {
	...
	static initModel = function() { // swap H, V because fuck me
		...
		var capHori = ceil(hori * ratio);
		if(capHori <= 0) return;

		var vt   = array_create(vert * capHori * 2 * 3);
		var vc   = array_create(vert * 3); // cap
		var topA = -90 + 180 * ratio;
		
		vertex  = [ vt, vc ];
		edges   = [];
		
	    for (var i = 0; i < vert;    i++)
	    for (var j = 0; j < capHori; j++) {
			var ha0 = (i + 0) / vert * 360;
			var ha1 = (i + 1) / vert * 360;
			var va0 = -90 + (j + 1) / hori * 180;
			var va1 = -90 + (j + 0) / hori * 180;
			    va0 = min(va0, topA);
		...
		var topH = dsin(topA) * 0.5;
		var topR = dcos(topA) * 0.5;
		for( var i = 0; i < vert; i++) {
			var ha0 =  i      / vert * 360;
			var ha1 = (i + 1) / vert * 360;
			
			vc[i*3+0] = new __vertex(dcos(ha1) * topR, dsin(ha1) * topR, topH);
			vc[i*3+1] = new __vertex(dcos(ha0) * topR, dsin(ha0) * topR, topH);
			vc[i*3+2] = new __vertex(0, 0, topH);

			vc[i*3+0].setNormal(0, 0, 1);
			vc[i*3+1].setNormal(0, 0, 1);
			vc[i*3+2].setNormal(0, 0, 1);

			vc[i*3+0].setUV(.5 + dcos(ha1) * .5, .5 + dsin(ha1) * .5);
			vc[i*3+1].setUV(.5 + dcos(ha0) * .5, .5 + dsin(ha0) * .5);
			vc[i*3+2].setUV(.5, .5);
		}
