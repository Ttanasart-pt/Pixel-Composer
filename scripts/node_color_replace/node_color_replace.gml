function Node_Color_replace(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	inputs[1] = nodeValue_Palette("Palette from", self, array_clone(DEF_PALETTE), "Color to be replaced.");
	
	inputs[2] = nodeValue_Palette("Palette to", self, array_clone(DEF_PALETTE), "Palette to be replaced to.");
	
	inputs[3] = nodeValue_Float("Threshold", self, 0.1)
		.setDisplay(VALUE_DISPLAY.slider);
	
	inputs[4] = nodeValue_Bool("Set others to black", self, false, "Set pixel that doesn't match any color in 'palette from' to black.");
	
	inputs[5] = nodeValue_Bool("Multiply alpha", self, true);
	
	inputs[6] = nodeValue_Bool("Hard replace", self, true, "Completely override pixel with new color instead of blending between it.");
