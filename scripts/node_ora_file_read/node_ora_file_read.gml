function Node_create_ORA_File_Read(_x, _y, _group = noone) { 
	...
	var node = new Node_ORA_File_Read(_x, _y, _group);
	node.skipDefault();
...
function Node_create_ORA_File_Read_path(_x, _y, path) { 
	if(!file_exists_empty(path)) return noone;
	
	var node = new Node_ORA_File_Read(_x, _y, PANEL_GRAPH.getCurrentContext());
	node.skipDefault();
	node.inputs[0].setValue(path);
	node.doUpdate();
...
function Node_ORA_File_Read(_x, _y, _group = noone) : Node(_x, _y, _group) constructor {
	name = "ORA File In";
	w    = 128;
	...
	static update = function(frame = CURRENT_FRAME) { 
		...
		var _width  = real(_meta.w);
		var _height = real(_meta.h);
