function Node_pSystem_3D_Follow_Path(_x, _y, _group = noone) : Node_3D(_x, _y, _group) constructor {
	name  = "Follow Path";
	icon  = THEME.vfx;
	...
	static processData = function(_outData, _data, _array_index = 0, _frame = CURRENT_FRAME) {
		...
		var _star = _data[ 6];
		var _stri = _data[ 7], _stri_curved = inputs[7].attributes.curved && curve_stri != undefined;
		var _spri = _data[ 9], _spri_curved = inputs[9].attributes.curved && curve_spri != undefined;
		...
		repeat(_partAmo) {
			...
			var _ppz    = buffer_peek( _partBuff, _start + PSYSTEM_OFF.pospz,  buffer_f64  );
			...
			if(_star) {
				__p = _path.getPointRatio(_path_rat - .01, 0, __p);
				var _x0 = __p.x;
				var _y0 = __p.y;
				var _z0 = __p.z;
				...
				var _pdy = _y1 - _y0;
				var _pdz = _z1 - _z0;
				var _pdl = sqrt(_pdx*_pdx + _pdy*_pdy + _pdz*_pdz );
				
				if(_pdl == 0) {
					_px = _x0 + _psx * _path_dev;
					_py = _y0 + _psy * _path_dev;
					_pz = _z0 + _psz * _path_dev;
					...
					point_rotate_3d(_psx, _psy, _psz, _pdx, _pdy, _pdz, _lif * _spri_cur, __r);
					
					_px = _x0 + __r[0] * _path_dev;
					_py = _y0 + __r[1] * _path_dev;
					_pz = _z0 + __r[2] * _path_dev;
				}
