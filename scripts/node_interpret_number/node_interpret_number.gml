#region
	FN_NODE_CONTEXT_INVOKE {
		addHotkey("Node_Interpret_Number", "Mode > Toggle", "M", MOD_KEY.none, function() /*=>*/ { PANEL_GRAPH_FOCUS_STR _n.inputs[1].setValue((_n.inputs[1].getValue() + 1) % 2); });
	});
#endregion
...
function Node_Interpret_Number(_x, _y, _group = noone) : Node_Processor(_x, _y, _group) constructor {
	...
	static drawOverlay = function(hover, active, _x, _y, _s, _mx, _my, _snx, _sny) {
		...
		var hv = inputs[5].drawOverlay(w_hoverable, active, _x, _y, _s, _mx, _my, _snx, _sny, getDimension()); OVERLAY_HV
