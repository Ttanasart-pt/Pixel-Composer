function Node_3D_Mesh_Wall_Builder(_x, _y, _group = noone) : Node_3D_Mesh(_x, _y, _group) constructor {
	...
	newInput(i+ 0, nodeValue_Path( "Path" )).setExtractNode("Node_Path_3D");
	...
	////- =Transform
	newInput(i+10, nodeValue_Bool(     "Flip Y", false ));
	...
	input_display_list = [
		...
		__d3d_input_list_transform, i+10, 
	...
	static processData = function(_output, _data, _array_index = 0, _frame = CURRENT_FRAME) {
		...
		for( var i = 0; i <= _segment; i++ ) {
			...
			points[i] = [ p.x * _pscale, p.y * _pscale * fy ];
		}
