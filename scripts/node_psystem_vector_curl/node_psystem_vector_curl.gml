function Node_pSystem_Vector_Curl(_x, _y, _group = noone) : Node(_x, _y, _group) constructor {
	name  = "Vector Curl";
	icon  = THEME.vfx;
	...
	static update = function(_frame = CURRENT_FRAME) {
		...
		temp_surface[1] = surface_verify(temp_surface[1], vw, vh, surface_rgba16float);
		...
		surface_set_shader(temp_surface[1], sh_psystem_curl);
			shader_set_2("dimension",  [vw, vh] );
			shader_set_i("oversample", _ovfr    );
			draw_surface(temp_surface[0], 0, 0);
		surface_reset_shader();
		
		var vectr_samp = new Surface_sampler(temp_surface[1]);
