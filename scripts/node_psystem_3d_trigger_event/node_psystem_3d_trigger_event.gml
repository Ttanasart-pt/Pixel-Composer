function Node_pSystem_3D_Trigger_Event(_x, _y, _group = noone) : Node_3D(_x, _y, _group) constructor {
	name  = "Trigger";
	icon  = THEME.vfx;
	color = COLORS.node_blend_vfx;
	setDrawIcon(s_node_psystem_3d_trigger_event);
	...
	static update = function(_frame = CURRENT_FRAME) { 
		...
		repeat(_partAmo) {
			...
			if((_step_cur == 0 || _lif % _step_cur == 0) && random(1) <= _chan_cur) {
				...
				var _vx = buffer_read_at( _partBuff, _start + PSYSTEM_OFF.velz, buffer_f64  );
				
				buffer_write(stepTrig, buffer_f64, _dx);
				buffer_write(stepTrig, buffer_f64, _dy);
				buffer_write(stepTrig, buffer_f64, _dz);
