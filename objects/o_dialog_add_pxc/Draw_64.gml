/// @description init
if !ready exit;

DIALOG_DRAW_BG

#region base UI
	draw_set_text(f_p2, fa_left, fa_top, COLORS._main_text_sub);
	draw_text(_dialog_x + ui(20), _dialog_y + ui(8), __txt("Import PXC as"));
#endregion

#region nodes
	draw_sprite_stretched(THEME.ui_panel_bg, 1, _dialog_x + ui(12), _dialog_y + title_h, dialog_w - ui(24), ui(120));
	var grid_size  = ui(64);
	var grid_space = ui(16);
	var grid_width = grid_size + grid_space;
	
	for(var i = 0; i < array_length(nodes); i++) {
		var _node = nodes[i];
		var xx    = _dialog_x + ui(32) + i * grid_width;
		var yy    = _dialog_y + title_h + ui(16);
		
		PANEL_GRAPH.stepBegin();
		
		var nx = PANEL_GRAPH.graph_cx;
		var ny = PANEL_GRAPH.graph_cy;
		
		draw_sprite_stretched_ext(THEME.node_bg, 0, xx, yy, grid_size, grid_size, COLORS.node_base_bg);
		draw_sprite_stretched_ext(THEME.node_bg, 1, xx, yy, grid_size, grid_size, CDEF.main_dark);
		
		if(sHOVER && point_in_rectangle(mouse_mx, mouse_my, xx, yy, xx + grid_width, yy + grid_size)) {
			draw_sprite_stretched_ext(THEME.node_bg, 1, xx, yy, grid_size, grid_size, COLORS._main_accent, 1);
			if(mouse_lpress(sFOCUS)) {
				
				switch(i) {
					case 0 : LOAD_PATH(path); break;
					case 1 : 
						var _n = Node_create_PXC_path(nx, ny, path);
						    _n.skipDefault();
						break;
				}
				
				instance_destroy();
			}
		}
		
		var _txt = "";
		
		switch(i) {
			case 0 :
				THEME.dFile_load.draw(xx + grid_size / 2, yy + grid_size / 2, 2 * UI_SCALE / THEME_SCALE);
				_txt = __txt("Load");
				break;
				
			case 1 : 
				draw_sprite_uniform(_node.getSpr(), 0, xx + grid_size / 2, yy + grid_size / 2, .5 * UI_SCALE);
				_txt = _node.name;
				break;
				
		}
		
		draw_set_text(f_p3, fa_center, fa_top, COLORS._main_text);
		draw_text_ext(xx + grid_size / 2, yy + grid_size + 4, _txt, -1, grid_size + grid_space / 2);	
	}
#endregion

DIALOG_DRAW_FOCUS